...
setTimeout(() => {
  addMessage(myName, '来月のどっかで会う？', 'right');

  setTimeout(() => {
    addMessage(partnerName, 'うん！会いたい！', 'left');
...
function askWhereToGo() {
  setTimeout(() => {
    ...
    setTimeout(() => {
      addMessage(partnerName, 'うーん、会えればどこでも嬉しいな！', 'left');
...
function saveAndRedirect() {
  const honneMap = {
    "〇日なら空いてるけどどう？": "本当はその日も会いたくないけど断るのも気まずいし、",
  "じゃあ月末とかで合わせようか": "できるだけ先延ばしにしたい。正直もう冷めてるし、",
  "とりあえず空いてる日わかったら連絡するわ": "会う気はないけど、断るのも面倒。自然消滅狙いでフェードアウトしよう。",
  "映画とかカフェとか？": "デートっぽいことすればごまかせるかな。気持ちは全然乗ってないけど。",
  "家でまったりでもいいけど": "外出るのも面倒。なんなら会わずに済めば最高。",
  "俺はどこでもいいから、君が行きたいとこでいいよ": "何も決めたくないし、責任も持ちたくない。",
  "昼から合流しよっか": "彼氏っぽいことして、うまくやり過ごすか。",
  "夕方からでもいい？": "最低限の時間で済ませたい。少しでも会う時間が短くなれば、",
  "直前でまた決めよう": "できればドタキャンしたい。ギリギリまで様子見て、うまく逃げたい。"
  };
